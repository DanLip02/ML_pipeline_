...
def apply_data():
    ...
    print(f"✅ Загружено {len(df)} строк и {df.shape[1]} столбцов.")

    X, y, num_features, cat_features = prepare_features(df, cfg_data)
    print(f"📊 Числовые фичи: {num_features}")
    print(f"🏷️ Категориальные фичи: {cat_features}")
    if y is not None:
        print(f"🎯 Целевая переменная: {cfg_data['data']['target']}")
